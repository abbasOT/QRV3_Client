...
function VisitorScreen() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://localhost:8000/commercialAdmin/get_ComAdmin/${com_prop_id}`
        );
        ...
        if (data.commercialAdmin.brightness) {
          setBrightness(data.commercialAdmin.brightness);
        }
      } catch (error) {
    ...
    if (e.key === "Enter") {
      try {
        ...
        const response = await axios.post(
          `https://localhost:8000/commercialAdmin/WelcomMessage/${com_prop_id}`,
          {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://localhost:8000/commercialAdmin/uploadWallpaper/${com_prop_id}`,
        formData
      );
  ...
  const saveBrightnessValue = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_URL1}/commercialAdmin/savebrightness/${com_prop_id}`,
        { brightness }
      )
      .then((response) => {
  ...
  return (
    ...
    <div>
          ...
          <div className="col-5">
            ...
            <div className="mt-5 ">
              <button
                style={btnStyle}
                type="button"
                className="btn btn-primary  mb-5 shadow"
                onClick={DownloadQR}
              >
      ...
      >
        <Modal.Body>
          <IntercomIdModal setInterIdModal={setInterIdModal} setCommercialData={setCommercialData} />
        </Modal.Body>
      </Modal>
    </div>
    </>
